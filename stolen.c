...
#define FILESIZ 131072 // Size of file, 128 MB
...
static void task_upload(task_t *t)
{
    ...
    // Task 2: check that filename is less than size limit
    if (strlen(t->filename) > FILENAMESIZ) {
        error("* Filename too long %s", t->filename);
        goto exit;
    }
    // Task 2: serve files only in local directory
    if ((strcmp(t->filename, "..") == 0) || (strcmp(t->filename, "~") == 0)) {
        error("* Invalid directory %s", t->filename);
        goto exit;
    }
	...
	if (evil_mode) {
		...
		while (1) {
			...
			if (ret == TBUF_ERROR) {
				...
				// Set head back to beginning of file
				t->head = beginning;
		}
	}
	// Good mode
...
"         -b[MODE]     Evil mode!!!!!!!!\n");
    ...
    // Task 1: parallel download and upload using fork
    pid_t child;
